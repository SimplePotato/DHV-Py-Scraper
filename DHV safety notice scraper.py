...
import time
    ...
    # The site we are scraping from is https://www.dhv.de/en/safety/safety-notes/


while True:     # While loop executing code every n seconds as it is set in line 96
    ...
    print("Today  " + todayDate())
        ...
        #fakeDate is for testing purposes only (when not in testing make change on line 86 at if statement)
    def fakeDate():
        td = date.today()
        fd = td.replace(day=9, month=8, year=2018)
    ...
    try:
        ...
        #Print out only if date is equal as todays date (Can choose betwen todayDate and fakeDate but this is for testing only)
            if (datum.text) == (fakeDate()):
                ...
                pass
    time.sleep(14400.0) # 14400 sec is 4h
